...
def m_norm(goal):
    def f(x):
        return max(p_norm(p, goal)(x) for p in range(1, 10))
...
class GUI(Tk):
    ...
    def get_heuristic(self, start):
        if self.he == 0:
            ...
            heuristic = m_norm(self.goal)
        elif self.he > 1000:
            heuristic = inf_norm(self.goal)
        elif self.he < 0:
            heuristic = wall_weight(self.goal, self.labirinth, -self.he)
        else:
            heuristic = lambda pos: p_norm(self.he, self.goal)(pos) * (1 + 0.01)
