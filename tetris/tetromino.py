...
def main():
    ...
    while True:  # game loop
        pygame.mixer.music.load("tetris{}.mid".format(random.choice("bc")))
...
def rotate(fallingPiece, amount):
    len_piece = len(PIECES[fallingPiece.shape])
    new_rotation = (fallingPiece.rotation + amount) % len_piece
    fallingPiece.rotation = new_rotation
...
def runGame():
    ...
    while True:  # game loop
        ...
        if movingLeft or movingRight:
            if time.time() - lastMoveSidewaysTime > MOVESIDEWAYSFREQ:
        ...
        if movingDown:
            if (time.time() - lastMoveDownTime > MOVEDOWNFREQ and
...
def getNewPiece():
    ...
    shape = random.choice(PIECES_STR)
    newPiece = Piece(shape,
                     random.randint(0, len(PIECES[shape]) - 1),
                     BOARDWIDTH // 2 - TEMPLATEWIDTH // 2,
...
def isValidPosition(board, p, adjX=0, adjY=0):
    # Return True if the piece is within the board and not colliding
    for x in range(TEMPLATEWIDTH):
        for y in range(TEMPLATEHEIGHT):
            isAboveBoard = y + p.y + adjY < 0
            if isAboveBoard or PIECES[p.shape][p.rotation][y][x] == BLANK:
                continue
            if not isOnBoard(x + p.x + adjX, y + p.y + adjY):
                return False
            if board[x + p.x + adjX][y + p.y + adjY] != BLANK:
                return False
    return True
