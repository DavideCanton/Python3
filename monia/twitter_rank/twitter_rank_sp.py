...
def compute_twitter_rank_topic(tp, dt, num_tweets, relations):
    ...
    row_sum = np.array(p_t.sum(1)).flatten()
    mask = np.where(np.logical_and(col_sum == 0, row_sum == 0), [1. / n], [0.])
    del row_sum
    del col_sum
    ...
    while True:
        tr_t = GAMMA * (p_t.dot(tr_t_pre) + p_t.T.dot(tr_t_pre) -
                        tr_t_pre * diag)
        tr_t += e_t + GAMMA * np.dot(mask, tr_t_pre)
