...
class Board:
    ...
    def useful(self, pos):
        ...
        return not dupe and (empties or weak_opps or
                             (strong_neighs and (strong_opps or weak_neighs)))
    ...
    def check(self):
        for square in self.squares:
            ...
            while changed:
                ...
                for member in members1.copy():
                    for neighbour in member.neighbours:
                        if neighbour.color == square.color and neighbour not \
                                in members1:
                            changed = True
            ...
            assert ledges1 == ledges2, (
                'ledges differ at %r: %d %d' % (square, ledges1, ledges2))

            empties1 = set(self.emptyset.empties)

            empties2 = set()
            for square_ in self.squares:
                if square_.color == EMPTY:
                    empties2.add(square_.pos)

    def __repr__(self):
        result = []
        for y in range(SIZE):
            start = to_pos(0, y)
            result.append(''.join([SHOW[square.color] + ' ' for square in
                                   self.squares[start:start + SIZE]]))
        return '\n'.join(result)
...
class UCTNode:
    ...
    def select(self, board):
        """ select move; unexplored children first, then according to uct
        value """
        if self.unexplored:
    ...
    def best_visited(self):
        ...
        for child in self.pos_child:
            # if child:
            # print(to_xy(child.pos), child.wins, child.losses,
            # child.score())
